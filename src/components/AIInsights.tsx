...
import { CheckCircle2, AlertTriangle, Sprout, Loader2 } from "lucide-react";
...
export const AIInsights = () => {
  ...
  useEffect(() => {
    ...
    const fetchAnalysisResults = async () => {
      try {
        // @ts-ignore - analysis_results table not yet in generated types
        ...
        if (data) {
          const generatedInsights = generateInsights(data as unknown as AnalysisResult);
  ...
  const generateInsights = (result: AnalysisResult): InsightMessage[] => {
    ...
    if (result.predicted_yield_kg !== null) {
      if (result.predicted_yield_kg > 5000) {
        messages.push({
          type: "success",
          message: t("aiInsights.yieldHigh"),
        });
      } else if (result.predicted_yield_kg < 3000) {
        messages.push({
          type: "warning",
          message: t("aiInsights.yieldLow"),
        });
      }
    }
    ...
    if (result.predicted_wastage_percent !== null && result.predicted_wastage_percent > 10) {
      messages.push({
        type: "warning",
        message: t("aiInsights.wastageHigh"),
      });
    }
    ...
    if (result.disease_risk_level === "Low") {
      messages.push({
        type: "success",
        message: t("aiInsights.diseaseRiskLow"),
      });
    }
    ...
    if (
      ...
      messages.push({
        type: "info",
        message: t("aiInsights.irrigationSchedule", {
    ...
    if (result.fertilizer_recommendation) {
      messages.push({
        type: "info",
        message: t("aiInsights.fertilizerPlan", {
    ...
    if (result.next_crop_suggestion) {
      ...
      messages.push({
        type: "info",
        message: t("aiInsights.nextCropSuggestion", {
          crop: translatedCrop,
        }),
      });
    }
